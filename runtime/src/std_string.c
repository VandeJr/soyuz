...
int64_t soyuz_str_starts_with(SoyuzString *s, SoyuzString *prefix) {
    ...
    return memcmp(soyuz_str_data(s), soyuz_str_data(prefix), (size_t)prefix->len) == 0 ? 1 : 0;
}
...
int64_t soyuz_str_ends_with(SoyuzString *s, SoyuzString *suffix) {
    ...
    if (pl > sl) return 0;
    return memcmp(soyuz_str_data(s) + sl - pl, soyuz_str_data(suffix), (size_t)pl) == 0 ? 1 : 0;
}
...
// Content equality: returns 1 if both strings have identical bytes, 0 otherwise.
int64_t soyuz_str_eq(SoyuzString *a, SoyuzString *b) {
    if (!a && !b) return 1;
    if (!a || !b) return 0;
    if (a->len != b->len) return 0;
    return memcmp(soyuz_str_data(a), soyuz_str_data(b), (size_t)a->len) == 0 ? 1 : 0;
}
