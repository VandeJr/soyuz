...
/*
 * Bootstrap compatibility: older generated IR prefixes static objects with
 * only { refcount, dtor }, while heap objects use the current full header.
 * Detect the static prefix before subtracting sizeof(SoyuzHeader), otherwise
 * retain/release read before the LLVM global and may corrupt the heap.
 */
static int soyuz_is_legacy_static(const void *ptr) {
    const int64_t *words = (const int64_t *)ptr;
    return words[-2] == SOYUZ_STATIC_REFCOUNT;
}

void soyuz_retain(void *ptr) {
    if (!ptr) return;
    if (soyuz_is_legacy_static(ptr)) return;
...
static uint64_t soyuz_hash(void *key, int64_t is_string) {
    if (is_string) {
        uint64_t hash = 5381;
        const SoyuzString *string = (const SoyuzString *)key;
        const unsigned char *bytes = (const unsigned char *)soyuz_str_data(string);
...
static int soyuz_key_eq(void *k1, void *k2, int64_t is_string) {
    if (is_string) {
        if (k1 == k2) return 1;
        if (!k1 || !k2) return 0;
        const SoyuzString *s1 = (const SoyuzString *)k1;
        const SoyuzString *s2 = (const SoyuzString *)k2;
        if (s1->len != s2->len) return 0;
        return memcmp(soyuz_str_data(s1), soyuz_str_data(s2), (size_t)s1->len) == 0;
    }
    return k1 == k2;
}
