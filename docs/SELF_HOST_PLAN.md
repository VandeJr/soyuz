...
| **S7** | Async e concorrência | `gen_channel.go`, `gen_sync.go`, `gen_arc.go`, `gen_gather.go` | Módulos correspondentes em `src/codegen/` | Testes M9–M26 (channel, sync, arc, task, gather) passam | `in_progress` |
