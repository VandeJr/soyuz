#!/usr/bin/env bash
# S9 bootstrap gate: CLI run dispatch for hello_minimal.sy → materialize → exec hello.
# Sequence mirrors cliHelloMinimalRun*Command helpers in src/driver/cli.sy.
set -euo pipefail
...
if [[ ! -f "$HELLO" ]]; then
  echo "fixture CLI não encontrado: $HELLO" >&2
  exit 1
fi
for script in "$EXPORT_SCRIPT" "$APPLY_SCRIPT" "$LINK_SCRIPT"; do
  if [[ ! -x "$ROOT/$script" ]]; then
    echo "script CLI run dispatch ausente ou não executável: $script" >&2
    exit 1
  fi
done
...
echo "→ driver cli-run-dispatch check (bootstrap) OK"
